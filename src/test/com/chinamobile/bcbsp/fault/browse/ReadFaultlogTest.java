...
public class ReadFaultlogTest {
	...
	String localDirPath = "src/test/com/chinamobile/bcbsp/fault/browse/log";
	...
	public void testReadDirWithKey() {
		System.out.println();
		System.out
				.println("----------------testReadDirWithKey----------------");
		List<Fault> listReadDir = readFaultlog.readDirWithKey(localDirPath,
				keys);
		for (Iterator iterator = listReadDir.iterator(); iterator.hasNext();) {
			Fault fault = (Fault) iterator.next();
			System.out.println(fault);
