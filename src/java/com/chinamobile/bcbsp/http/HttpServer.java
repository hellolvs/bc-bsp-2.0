...
public class HttpServer {
  ...
  /** Define constant for get the initializers */
  static final String FILTER_PROPERTY = "bcbsp.http.filter.initializers";
  /** My web server */
  protected final Server webServer;
  /** Connector for connecting web server */
  protected final Connector listener;
  /** Define WebAppContext for BSP web */
  protected final WebAppContext webAppContext;
  /** Define boolean for judging whether exist port or not */
   ...
   * @param findPort
   *        whether the server should start at the given port and increment by 1
   ...
   * @param bindAddress The IP address of the server
  ...
  public HttpServer(String name, String bindAddress, int port,
    ...
    final String appDir = conf.get("bsp.http.infoserver.webapps",
   ...
   * listener.
   * @param conf Configuration.
   * @return ret return SelectChannelConnector
   */
  ...
  protected void addDefaultApps(ContextHandlerCollection parent,
    ...
    /** Set up context for "/logs/" if "bcbsp.log.dir" property is defined */
    String logDir = System.getProperty("bcbsp.log.dir");
    if (logDir != null) {
   ...
   * Add Context.
   * @param ctxt Context
   * @param isFiltered whether is filted or not
   */
      ...
      while (true) {
        try {
          ...
          if (port < 0) {
            ...
            int numRetries = 1;
            while (port < 0) {
              LOG.info("listener.getLocalPort returned " + port);
          ...
          port = listener.getLocalPort();
          if (port < 0) {
            LOG.info("Bounds port is " + port + " after webserver start");
            ...
            if (port < 0) {
              throw new Exception("listener.getLocalPort() is returning " +
                "less than 0 even after " + maxRetries  + " resets");
            }
          }
          ...
          if (ex instanceof BindException) {
            ...
            LOG.error("HttpServer.start() threw a non Bind IOException");
            throw ex;
          }
        } catch (MultiException ex) {
          LOG.error("HttpServer.start() threw a MultiException");
          throw ex;
        }
        oriPort = oriPort + 1;
        listener.setPort(oriPort);
