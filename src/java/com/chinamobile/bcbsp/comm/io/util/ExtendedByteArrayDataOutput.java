...
/**
 * Adds some functionality to ByteArrayOutputStream, such as an option to write
 * int value over previously written data and directly get the byte array.
 */
public class ExtendedByteArrayDataOutput extends ByteArrayOutputStream
  ...
  /**
   * Uses the byte array provided at the given position.
   * @param buf
   *        Buffer to use
   * @param pos
   *        Position in the buffer to start writing from
   */
  ...
  public void setPos(int pos) {
    /*
     * if(pos < 0){ this.count = 0; }else if(pos>this.buf.length){ this.count =
     * this.buf.length -1; }
     */
    this.count = pos;
  }
