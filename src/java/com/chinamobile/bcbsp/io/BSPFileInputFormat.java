...
/**
 * BSPFileInputFormat This class is used for reading from the file system, such
 * as HDFS.
 */
public abstract class BSPFileInputFormat<K, V> extends InputFormat<K, V> {
  ...
  /** Define a overflow value of split. If the remaining is
   * less than 1.1 times a split, residual as a split. */
  private static final double SPLIT_SLOP = 1.1; // 10% slop
  /** Define a hidden file filter. */
  ...
  private static class MultiPathFilter implements PathFilter {
    /** Define An array to storage the PathFilter class use for
     * filtering the input paths. */
    private List<PathFilter> filters;

    /**
     * Set array of PathFilter.
     *
     * @param filters
    ...
    /**
     * To determine whether a path to be accepted by the PathFilter.
     *
   ...
   * the tasks while they are running.
  ...
  public static PathFilter getInputPathFilter(JobContext context) {
    ...
    Class<?> filterClass = conf.getClass("mapred.input.pathFilter.class", null,
        PathFilter.class);
    return (filterClass != null) ? (PathFilter) ReflectionUtils.newInstance(
        filterClass, conf) : null;
  }
    ...
    for (FileStatus file : listStatus(job)) {
      ...
      if ((length != 0) && isSplitable(job, path)) {
        ...
        if (job.getInt(Constants.USER_BC_BSP_JOB_SPLIT_FACTOR, 1) == 1) {
          ...
          if (job.getSplitSize() == 0L) {
            splitSize = blockSize *
                job.getInt(Constants.USER_BC_BSP_JOB_SPLIT_FACTOR, 1);
          } else {
            splitSize = job.getSplitSize() *
                job.getInt(Constants.USER_BC_BSP_JOB_SPLIT_FACTOR, 1);
          }
   ...
   * @param minSize
   *            The minimum value of each block
   * @param maxSize
   *            The maximum  value of each block
   * @return MinSize and the size of the minimum value,
   *            suppose blockSize and maxSize minimum value for the size.
   */
  ...
  public static void addInputPath(BSPJob job, Path path) throws IOException {
    ...
    conf.set(Constants.USER_BC_BSP_JOB_INPUT_DIR, dirs == null ? dirStr : dirs +
        "," + dirStr);
  }
