 ...
 */
public class TableInputFormatBase extends InputFormat<Text, Text> {
  ...
  // 2013.4.16
  // Reason: The code changes because of the new version of HBase 1.94.x
  @Override
  public List<InputSplit> getSplits(BSPJob job) throws IOException {
    if (table == null) {
      throw new IOException("No table was provided.");
    }
    Set<byte[]> set = table.getTableDescriptor().getFamiliesKeys();
    if (set.size() == 0) {
      throw new IOException("Expecting at least one record.");
    }
    ResultScanner sc = table.getScanner((byte[]) set.toArray()[0]);
    ...
    byte[] endRow = null;
    for (Result r : sc) {
      // tmpLocation = table.getRegionLocation(new
      // String(r.getRow())).getHostName();
      tmpLocation = table.getRegionLocation(new String(r.getRow()))
          .getRegionInfo().getRegionNameAsString();
      if (startRow == null) {
        startRow = r.getRow();
        regionLocation = tmpLocation;
      }
      if (tmpLocation.equals(regionLocation)) {
        endRow = r.getRow();
      } else {
        InputSplit split = new TableSplit(table.getTableName(), startRow,
    ...
    InputSplit split = new TableSplit(table.getTableName(), startRow, endRow,
        regionLocation);
    splits.add(split);
    return splits;
  }
  ...
  /*
   * public List<InputSplit> getSplits(BSPJob job) throws IOException { if
   * (table == null) { throw new IOException("No table was provided."); }
   * Pair<byte[][], byte[][]> keys = table.getStartEndKeys(); if (keys == null
   * || keys.getFirst() == null || keys.getFirst().length == 0) { throw new
   * IOException("Expecting at least one region."); } int count = 0;
   * List<InputSplit> splits = new ArrayList<InputSplit>(
   * keys.getFirst().length); for (int i = 0; i < keys.getFirst().length; i++) {
   * if (!includeRegionInSplit(keys.getFirst()[i], keys.getSecond()[i])) {
   * continue; } String regionLocation =
   * table.getRegionLocation(keys.getFirst()[i])
   * .getServerAddress().getHostname(); byte[] startRow = scan.getStartRow();
   * byte[] stopRow = scan.getStopRow(); // determine if the given start an stop
   * key fall into the region if ((startRow.length == 0 ||
   * keys.getSecond()[i].length == 0 || Bytes .compareTo(startRow,
   * keys.getSecond()[i]) < 0) && (stopRow.length == 0 ||
   * Bytes.compareTo(stopRow, keys.getFirst()[i]) > 0)) { byte[] splitStart =
   * startRow.length == 0 || Bytes.compareTo(keys.getFirst()[i], startRow) >= 0
   * ? keys .getFirst()[i] : startRow; byte[] splitStop = (stopRow.length == 0
   * || Bytes.compareTo( keys.getSecond()[i], stopRow) <= 0) &&
   * keys.getSecond()[i].length > 0 ? keys.getSecond()[i] : stopRow; InputSplit
   * split = new TableSplit(table.getTableName(), splitStart, splitStop,
   * regionLocation); splits.add(split); if (LOG.isDebugEnabled())
   * LOG.debug("getSplits: split -> " + (count++) + " -> " + split); } } return
   * splits; }
   */
   ...
   * exclude an entire region from the BSP job, (and hence, not contributing to
   ...
   * addition to reducing InputSplits, reduces the load on the region server as
   * well, due to the ordering of the keys. <br>
   * <br>
