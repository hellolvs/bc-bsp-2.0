...
/**
 * RandomVariable The RandomVaraibale Class provides static methods for
...
public class RandomVariable {
  ...
  public static double dirac(double[] values, double[] prob) {
    double[] probCumul = new double[values.length];
    probCumul[0] = prob[0];
    for (int i = 1; i < values.length; i++) {
      probCumul[i] = probCumul[i - 1] + prob[i];
    }
    double y = rand();
    double x = 0;
    for (int i = 0; i < values.length - 1; i++) {
      if ((y > probCumul[i]) && (y < probCumul[i + 1])) {
        x = values[i];
      }
    }
    return x;
